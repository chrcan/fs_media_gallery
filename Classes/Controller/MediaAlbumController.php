...
class MediaAlbumController extends ActionController
{
    ...
    protected function initializeAction(): void
    {
        ...
        // startingpoint/storagePid is not set via TS nor flexforms > fallback to current pid
            ...
            // KORREKTUR für TYPO3 v14: $GLOBALS['TSFE'] ist deprecated → verwende TYPO3_REQUEST
            $request = $GLOBALS['TYPO3_REQUEST'] ?? null;
            if ($request !== null) {
                $this->settings['startingpoint'] = $frameworkSettings['persistence']['storagePid'] =
                    $request->getQueryParams()['id'] ?? $frameworkSettings['persistence']['storagePid'];
            } else {
                // Fallback für Backend-Kontext oder andere Fälle
                $this->settings['startingpoint'] = $frameworkSettings['persistence']['storagePid'] =
                    $frameworkSettings['persistence']['storagePid'] ?? 0;
            }
        }
    ...
    {
        $mediaAlbum = $mediaAlbum !== null ? (int)$mediaAlbum : null;
        if (empty($mediaAlbum)) {
            $mediaAlbum = (int)($this->settings['mediaAlbum'] ?? 0) ?: null;
        }

        // if album uid is set through settings (typoscript or flexform) we skip the storage check
        $respectStorage = true;
        $settingsMediaAlbum = (int)($this->settings['mediaAlbum'] ?? 0);

        if ($settingsMediaAlbum > 0 && $mediaAlbum !== null && $settingsMediaAlbum === $mediaAlbum) {
            $respectStorage = false;
        }

        $album = $mediaAlbum !== null ? $this->mediaAlbumRepository->findByUid($mediaAlbum, $respectStorage) : null;
        $this->view->assign('mediaAlbum', $album);
        $this->view->assign('showBackLink', false);

        if ($album) {
            $this->view->assign('mediaAlbumPagination', $this->getAlbumPagination($album));
        }
    ...
    protected function pageNotFound(string $message): ResponseInterface
    {
        ...
        if ($request !== null) {
            try {
                ...
                // Falls ErrorController fehlschlägt, normale HTML-Antwort zurückgeben
                // Optional: Logging hinzufügen wenn benötigt
            }
        }

        // Fallback: Normale HTML-Antwort mit Fehlermeldung
