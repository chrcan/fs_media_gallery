...
class MediaAlbum extends AbstractEntity
{
    ...
    // KORREKTUR: Dependency Injection im Konstruktor
    public function __construct(
        ?FileCollectionRepository $fileCollectionRepository = null,
        ?MediaAlbumRepository $mediaAlbumRepository = null
    ) {
        // KORREKTUR: Repositories über Dependency Injection oder GeneralUtility mit Argumenten
        if ($fileCollectionRepository !== null) {
            $this->fileCollectionRepository = $fileCollectionRepository;
        } else {
            // TYPO3 v14: FileCollectionRepository benötigt ConnectionPool und FileCollectionRegistry
            $this->fileCollectionRepository = GeneralUtility::makeInstance(
                FileCollectionRepository::class,
                GeneralUtility::makeInstance(ConnectionPool::class),
                GeneralUtility::makeInstance(FileCollectionRegistry::class)
            );
        }

        if ($mediaAlbumRepository !== null) {
    ...
    // KORREKTUR: initializeObject angepasst
    public function initializeObject(): void
    {
        // KORREKTUR: FileCollectionRepository mit erforderlichen Abhängigkeiten
        ...
        if ($this->mediaAlbumRepository === null) {
            $this->mediaAlbumRepository = GeneralUtility::makeInstance(MediaAlbumRepository::class);
        }
    }

    // KORREKTUR: Getter für FileCollectionRepository mit Lazy-Loading
    protected function getFileCollectionRepository(): FileCollectionRepository
    {
        if ($this->fileCollectionRepository === null) {
            $this->fileCollectionRepository = GeneralUtility::makeInstance(
                FileCollectionRepository::class,
    ...
    // KORREKTUR: Getter für MediaAlbumRepository mit Lazy-Loading
    protected function getMediaAlbumRepository(): MediaAlbumRepository
    {
        if ($this->mediaAlbumRepository === null) {
            $this->mediaAlbumRepository = GeneralUtility::makeInstance(MediaAlbumRepository::class);
        }
        return $this->mediaAlbumRepository;
    }
    ...
    public function getAssets()
    {
        if ($this->assetCache === null) {
            try {
                // KORREKTUR: Getter verwenden statt direkter Property-Zugriff
                $fileCollectionRepository = $this->getFileCollectionRepository();

                /** @var AbstractFileCollection $fileCollection */
                $fileCollection = $fileCollectionRepository->findByUid($this->getUid());
                ...
                $this->getMediaAlbumRepository()->update($this);
                $this->assetCache = [];
            }
        }
    ...
    public function getAlbums()
    {
        if ($this->albumCache === null) {
            // KORREKTUR: Getter verwenden
            $this->albumCache = $this->getMediaAlbumRepository()->findByParentAlbum($this, $this->excludeEmptyAlbums);
        }
